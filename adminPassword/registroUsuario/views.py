...
def registroCredencial(request):
    ...
    pwdusuario=request.session.get('pwd').strip()
    if request.method=='GET':
        return render(request,template)
    if request.method=='POST':
        ...
        try:
            ...
            print('4')
            llave=Api.generar_llave_aes_from_password(pwdusuario)
            print('5')
            iv_bin=Api.generar_iv() 
            print('6')
            iv_texto=Api.bin_str(iv_bin)
            print('7')
            password_bin=Api.str_bin(passwordCredencial)
            print('8')
            password_cifrado_bin=Api.cifrar(password_bin,llave,iv_bin)
            print('9')
            ...
            error={"se ha presentado un error en no se donde"}
            print(error)
...
def usuario(request):
    ...
    if request.method=='GET': 
        ...
        for cuenta in cuentas:
            ...
            password_texto=Api.bin_str(password_desifrado_bin)
